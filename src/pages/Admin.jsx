...
const ADMIN_EMAILS = ['wnguetsop@gmail.com', 'admin@tontine.com']; // ⚠️ MODIFIER
...
export default function Admin() {
  ...
  const [filter, setFilter] = useState('all'); // all|free|pro|pending
  ...
  useEffect(() => {
    ...
    // Charger les demandes d'activation
  ...
  const handleActivatePro = async (userId, requestId = null) => {
    ...
    try {
      ...
      // Mettre à jour la demande si existe
      if (requestId) {
  ...
  // Badge clignotant pour nouveaux PRO
  const isNewPro = (userDate) => {
    if (!userDate) return false;
    const date = new Date(userDate);
    const now = new Date();
    const diff = (now - date) / (1000 * 60 * 60); // heures
    return diff < 24;
  };
  ...
  if (!isAdmin) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="bg-rose-50 border-2 border-rose-200 p-8 rounded-3xl text-center">
          <p className="text-3xl mb-4">🔒</p>
          <p className="font-black text-rose-800 text-xl mb-2">Accès Refusé</p>
          <p className="text-rose-600">Cette page est réservée aux administrateurs.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <div className="bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 rounded-3xl p-8 text-white shadow-2xl">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-white/80">Gestion des clients et abonnements</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {filteredUsers.map((u) => {
                ...
                const showNewBadge = isNewPro(u.updated_at) && u.subscription_status === 'pro';
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-bold text-slate-600">
                        {u.subscription_expires_at
                          ? new Date(u.subscription_expires_at).toLocaleDateString('fr-FR')
                          : '-'}
                      </p>
                    </td>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h3 className="font-black text-lg mb-4">💳 Paiements Récents</h3>
        <div className="space-y-2">
          {payments.slice(0, 10).map((p) => {
            const payUser = users.find(u => u.id === p.userId);
            return (
              <div key={p.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                <div>
                  <p className="font-bold">{payUser?.email || 'Utilisateur inconnu'}</p>
                  <p className="text-xs text-slate-500">
                    {new Date(p.created_at).toLocaleDateString('fr-FR')} • {p.payment_method}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-black text-emerald-600">{p.amount}€</p>
                  <span className={`text-xs font-bold ${
                    p.status === 'completed' ? 'text-emerald-600' : 'text-orange-600'
                  }`}>
                    {p.status === 'completed' ? '✓ Confirmé' : '⏳ En attente'}
                  </span>
                </div>
              </div>
            );
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-2">
                  💡 Conseil: Ajouter 30 jours pour un mois
                </p>
              </div>
